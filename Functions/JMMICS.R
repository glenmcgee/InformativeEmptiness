...
JMMICS_fit <- function(Nk,Zk,Yki,Xki,IDk,Z0k=NULL,weights=NA,minNk=0,NegBin=FALSE,ZIP=FALSE,slopes=FALSE,slope_col=2,condSize=FALSE,condOut=FALSE,joint=TRUE,nquad=25){
  ...
  if(joint==TRUE){ ## usual joint model
    ...
    if(slopes==FALSE){ ## random intercepts
      startval <- c(rep(1,ncol(Zk)),-1,rep(0,ncol(Xki)),log(0.5))
      ...
      startval <- c(rep(1,ncol(Zk)),-1,-1,rep(0,ncol(Xki)),log(0.5),log(0.5)) 
