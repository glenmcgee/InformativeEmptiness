...
outputsize <- 77 ## width of output (ie number of variables reported) DO NOT CHANGE UNLESS MODELS/FITS ARE CHANGED
...
fit_mods_cond <- function(gendat,size_plus=0,ZIP=FALSE){
  ...
  jno0s <- JMMICS_fit(Nk=df2$Nk[IDk_non0],Zk=cbind(1,df2$X1k)[IDk_non0,],Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),IDk=as.numeric(as.factor(df$IDki)),minNk=size_plus,ZIP=ZIP,condSize=TRUE,condOut=TRUE,nquad = 60)
  ...
  ## Joint Model
  joint <- JMMICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),IDk=df$IDki,minNk=size_plus,ZIP=ZIP,condSize=TRUE,condOut=TRUE,nquad = 60)
...
#############################
##  Fit marginal models
fit_mods_marg <- function(gendat,size_plus=0,ZIP=FALSE){
  df <- gendat$data; df2 <- gendat$data_clstlvl
  ...
  ## JMMICS
  jmm <- JMMICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),IDk=df$IDki,minNk=size_plus,ZIP=ZIP,condSize=FALSE,condOut=FALSE,nquad = 60)
...
simICSfit <- function(gam=0, ## scaling factor in volume model 
  ...
  cdat <- genICS(gamma_bk=gam,min_Nk=m,cond=TRUE,   K=KK,Xrate=Xprev,alpha0=a0,alpha1=a1,beta0=b0,beta1=b1,beta2=b2,sig.bk=sigma) ## conditional
  mdat <- genICS(gamma_bk=gam,min_Nk=m,cond=FALSE,  K=KK,Xrate=Xprev,alpha0=a0,alpha1=a1,beta0=b0,beta1=b1,beta2=b2,sig.bk=sigma) ## marginal
  ...
  ##if(ncol(res)!=77){ return(c(iter,rep(NA,77-1)))} ## if incorrect number of results
  ...
  colnames(res) <- c("*iter",
                     ...
                     "A0est_joint","A1est_joint","Gest_joint","B0est_joint","B1est_joint","B2est_joint","Sigest_joint","A0SE_joint","A1SE_joint","GSE_joint","B0SE_joint","B1SE_joint","B2SE_joint","SigSE_joint","MSEP_joint",
                     "*N_cond","*N0s_cond",
                     ...
                     "A0est_jmm","A1est_jmm","Gest_jmm","B0est_jmm","B1est_jmm","B2est_jmm","Sigest_jmm","A0SE_jmm","A1SE_jmm","GSE_jmm","B0SE_jmm","B1SE_jmm","B2SE_jmm","SigSE_jmm","MSEP_jmm",
                     "*N_marg","*N0s_marg")
...
for (rr in loopvec){
  ...
  res_gam25_m1 <- rbind(res_gam25_m1,simICSfit(gam=-0.25,m=1,iter=rr))
  res_gam10_m1 <- rbind(res_gam10_m1,simICSfit(gam=-0.10,m=1,iter=rr))
  res_gam00_m1 <- rbind(res_gam00_m1,simICSfit(gam=0,m=1,iter=rr))
  # res_gamP10_m1 <- rbind(res_gamP10_m1,simICSfit(gam=0.10,m=1,iter=rr))
  # res_gamP25_m1 <- rbind(res_gamP25_m1,simICSfit(gam=0.25,m=1,iter=rr))
...
write.table(res_gam25_m1,file=paste0(path,"results_poisson_gam25_m1",suffix,".txt"))
write.table(res_gam10_m1,file=paste0(path,"results_poisson_gam10_m1",suffix,".txt"))
write.table(res_gam00_m1,file=paste0(path,"results_poisson_gam00_m1",suffix,".txt"))

write.table(res_gam25_m0,file=paste0(path,"results_poisson_gam25_m0",suffix,".txt"))
write.table(res_gam10_m0,file=paste0(path,"results_poisson_gam10_m0",suffix,".txt"))
write.table(res_gam00_m0,file=paste0(path,"results_poisson_gam00_m0",suffix,".txt"))
