...
outputsize <- 91 #77 ## width of output (ie number of variables reported) DO NOT CHANGE UNLESS MODELS/FITS ARE CHANGED
...
fit_mods_cond <- function(gendat,size_plus=0,ZIP=FALSE){
  ...
  jno0s <- JMMICS_fit(Nk=df2$Nk[IDk_non0],Zk=cbind(1,df2$X1k)[IDk_non0,],Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),IDk=as.numeric(as.factor(df$IDki)),minNk=size_plus,ZIP=ZIP,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,nquad = 60)
  ...
  ## Joint Model
  joint <- JMMICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),IDk=df$IDki,minNk=size_plus,ZIP=ZIP,slopes=TRUE,slope_col=2,condSize=TRUE,condOut=TRUE,nquad = 60)
...
fit_mods_marg <- function(gendat,size_plus=0,ZIP=FALSE){
  ...
  ## JMMICS
  jmm <- JMMICS_fit(Nk=df2$Nk,Zk=cbind(1,df2$X1k),Yki=df$Yki,Xki=cbind(1,df$X1ki,df$X2ki),IDk=df$IDki,minNk=size_plus,ZIP=ZIP,slopes=TRUE,slope_col=2,condSize=FALSE,condOut=FALSE,nquad = 60)
...
simICSfit <- function(gam=0, ## scaling factor in volume model 
  ...
  cdat <- genICS(gamma_bk=gam,gamma1_bk=gam,min_Nk=m,slopes=TRUE,cond=TRUE,   K=KK,Xrate=Xprev,alpha0=a0,alpha1=a1,beta0=b0,beta1=b1,beta2=b2,sig.bk=sigma0,sig1.bk=sigma1) ## conditional
...
write.table(res_gam25_m1,file=paste0(path,"results_poisson_slopes_gam25_m1",suffix,".txt"))
write.table(res_gam10_m1,file=paste0(path,"results_poisson_slopes_gam10_m1",suffix,".txt"))
write.table(res_gam00_m1,file=paste0(path,"results_poisson_slopes_gam00_m1",suffix,".txt"))

write.table(res_gam25_m0,file=paste0(path,"results_poisson_slopes_gam25_m0",suffix,".txt"))
write.table(res_gam10_m0,file=paste0(path,"results_poisson_slopes_gam10_m0",suffix,".txt"))
write.table(res_gam00_m0,file=paste0(path,"results_poisson_slopes_gam00_m0",suffix,".txt"))
